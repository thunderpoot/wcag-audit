...
analyze = SourceFileLoader("analyze", os.path.join(
...
def test_parse_hex():
    ...
    assert_equal(analyze.parse_hex("#fff"), (255, 255, 255))
    assert_equal(analyze.parse_hex("#000"), (0, 0, 0))
    assert_equal(analyze.parse_hex("#f00"), (255, 0, 0))
    assert_equal(analyze.parse_hex("#ffffff"), (255, 255, 255))
    assert_equal(analyze.parse_hex("#000000"), (0, 0, 0))
    assert_equal(analyze.parse_hex("#ff6600"), (255, 102, 0))
    assert_equal(analyze.parse_hex("#336699"), (51, 102, 153))
    # With alpha
    assert_equal(analyze.parse_hex("#ff660080"), (255, 102, 0))
    assert_equal(analyze.parse_hex("#f008"), (255, 0, 0))
    print("  PASS: test_parse_hex")


def test_parse_rgb():
    """Test rgb/rgba color parsing."""
    assert_equal(analyze.parse_rgb("rgb(255, 0, 0)"), (255, 0, 0))
    assert_equal(analyze.parse_rgb("rgb(0, 128, 255)"), (0, 128, 255))
    assert_equal(analyze.parse_rgb("rgba(255, 0, 0, 0.5)"), (255, 0, 0))
    assert_equal(analyze.parse_rgb("rgb(100%, 0%, 0%)"), (255, 0, 0))
...
def test_parse_named_colors():
    ...
    assert_equal(analyze.parse_color("red"), (255, 0, 0))
    assert_equal(analyze.parse_color("white"), (255, 255, 255))
    assert_equal(analyze.parse_color("black"), (0, 0, 0))
    assert_equal(analyze.parse_color("navy"), (0, 0, 128))
    assert_equal(analyze.parse_color("cornflowerblue"), (100, 149, 237))
    assert_equal(analyze.parse_color("rebeccapurple"), (102, 51, 153))
    print("  PASS: test_parse_named_colors")


def test_parse_color_edge_cases():
    """Test edge cases in color parsing."""
    assert_equal(analyze.parse_color("transparent"), None)
    assert_equal(analyze.parse_color("inherit"), None)
    assert_equal(analyze.parse_color("currentColor"), None)
    assert_equal(analyze.parse_color("initial"), None)
    assert_equal(analyze.parse_color(""), None)
    assert_equal(analyze.parse_color(None), None)
    assert_equal(analyze.parse_color("not-a-color"), None)
...
def test_relative_luminance():
    ...
    assert_close(analyze.relative_luminance((255, 255, 255)), 1.0)

    # Black should have luminance 0.0
    assert_close(analyze.relative_luminance((0, 0, 0)), 0.0)

    # Known values from WCAG spec examples
    # Mid-gray
    lum = analyze.relative_luminance((128, 128, 128))
...
def test_contrast_ratio():
    ...
    ratio = analyze.contrast_ratio(white, black)
    assert_close(ratio, 21.0, tol=0.1)

    # Same color: minimum contrast 1:1
    ratio = analyze.contrast_ratio(white, white)
    ...
    gray = (118, 118, 118)
    ratio = analyze.contrast_ratio(gray, white)
    assert_close(ratio, 4.54, tol=0.1, msg=f"Gray on white: {ratio}")

    # Red on white
    red = (255, 0, 0)
    ratio = analyze.contrast_ratio(red, white)
...
def test_contrast_ratio_symmetry():
    ...
    c2 = (200, 200, 50)
    assert_close(
        analyze.contrast_ratio(c1, c2),
        analyze.contrast_ratio(c2, c1)
    )
...
def test_extract_colors_from_css():
    ...
    """
    decls = analyze.extract_colors_from_css(css)
...
def test_analyze_html_basic():
    ...
    print("  PASS: test_analyze_html_basic")


def test_analyze_html_no_colors():
    ...
    print("  PASS: test_analyze_html_no_colors")


def test_analyze_html_inline_only():
    ...
    print("  PASS: test_analyze_html_inline_only")
...
def test_wcag_threshold_boundary():
    ...
    gray = analyze.parse_color("#767676")
    white = (255, 255, 255)
    ratio = analyze.contrast_ratio(gray, white)
    ...
    lighter_gray = analyze.parse_color("#777777")
    ratio2 = analyze.contrast_ratio(lighter_gray, white)
...
def run_all():
    ...
    tests = [
        ...
        test_analyze_html_basic,
        test_analyze_html_no_colors,
        test_analyze_html_inline_only,
